...
class SingleBubblePlotter(MatplotlibPlotter):
    def define_plot(self):
        ...
        self.add_plot("liquid/temperature", colorbar=cb_T)
...
class SingleBubbleProblem(Problem):
    def __init__(self):
        ...
        self.sigma00=0.0721*newton/meter # surface tension at c=0
    ...
    def define_problem(self):
        ...
        liquid_eqs+=NavierStokesFreeSurface(surface_tension=self.surface_tension, mass_transfer_rate=self.mass_transfer_rate*10000)@"interface"  #, static_interface=True
        ...
        liquid_eqs+=ConnectFieldsAtInterface("c")@"interface"
        ...
        bubble_eqs+=DirichletBC(c=self.csat)@"interface"
        bubble_eqs+=InitialCondition(c=self.csat)
        #bubble_eqs+=InitialCondition(pressure=(2)*self.sigma00/self.R0)
    ...
    def presolve_temperature(self):
        ...
        with self.select_dofs() as dofs:
            dofs.select("liquid/temperature","substrate/temperature" ,"liquid/liquid_substrate/_lagr_conn_temperature_temperature") #,"substrate/temperature"  , "liquid/liquid_substrate/_lagr_conn_temperature_temperature"
            dofs.select("liquid/c","bubble/c","liquid/interface/_lagr_conn_c_c")
            self.solve()
