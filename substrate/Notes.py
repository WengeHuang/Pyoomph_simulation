#Bubble setup with only the Navier-Stokes Equation 
...
with SingleBubbleLaserProblem() as p:
    p.run(5*micro*second, outstep=0.1*micro*second, temporal_error=1)


#Static bubble with NS equation, temperature and concentration field
...
class SingleBubblePlotter(MatplotlibPlotter):
    def define_plot(self):
        ...
        self.add_plot("liquid/c",colorbar=cb_u)
...
class SingleBubbleProblem(Problem):
    ...
    def presolve_temperature(self):
        ...
        with self.select_dofs() as dofs:
            dofs.select("liquid/temperature")
